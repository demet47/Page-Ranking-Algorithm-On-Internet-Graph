...
int main(int arg, char* argv[])
{
    ...
    for (int schedule = 0; schedule < 3; schedule++){
        for(int chunk =1000; chunk < 6000; chunk+=2000){
            ...
            csvFile<<testNo<<","<<schedule_name<<","<<chunk<<",";
            for (int tread = 1; tread < 9; tread++){
                    omp_set_num_threads(tread);
                    ...
                    if (tread == 1){
                        ...
                        csvFile<<endTime-beginTime<<endl;
                        testNo++;
